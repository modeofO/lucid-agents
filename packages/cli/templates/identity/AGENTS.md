...
- **Sets the tokenURI** to `https://{domain}/.well-known/agent-metadata.json`
- Retrieves trust configuration for the manifest

**⚠️ CRITICAL: After registration, you MUST host ERC-8004 metadata at the tokenURI**

When you register an agent, the ERC-8004 registry stores a `tokenURI` pointing to your metadata. This URI is set to:
```
https://{your-domain}/.well-known/agent-metadata.json
```

**Why this matters:**
- The `tokenURI` is stored **on-chain** in the ERC-8004 registry contract
- Anyone querying the registry will fetch metadata from this URL to verify your agent's identity
- Without hosting the metadata file, your agent's on-chain identity **cannot be verified**
- The metadata must match the ERC-8004 format (not Agent Card format)

The template automatically generates and displays the metadata JSON after registration. You must:
1. Copy the generated metadata JSON
2. Host it at the exact tokenURI location (`/.well-known/agent-metadata.json`)
3. Ensure it's accessible over HTTPS with `Content-Type: application/json`

## ERC-8004 Metadata vs Agent Card

**Important:** There are two different metadata formats that serve different purposes:

### ERC-8004 Metadata (`/.well-known/agent-metadata.json`)
...
- **Generated by:** `generateAgentMetadata()` from `@lucid-agents/identity`
...
## Hosting ERC-8004 Metadata

After registration, you **MUST** host the ERC-8004 metadata at the tokenURI. The template automatically generates this for you:
...
**Example ERC-8004 metadata format:**
```json
{
  "name": "My Agent",
  "description": "An intelligent assistant",
...
2. Anyone querying the registry will fetch metadata from this URL to verify your agent's identity
3. Without hosting the file, your agent's on-chain identity **cannot be verified**
4. The metadata must be accessible over HTTPS with proper `Content-Type: application/json`
...
- `/.well-known/agent-metadata.json` - ERC-8004 metadata (you must host this manually)
